const StoreBuilder = (function () {
  ...
  function bindEvents() {
    ...
    // Mobile/Click Support: Click component to add directly
    document.addEventListener("click", function (e) {
      const componentItem = e.target.closest(".component-item");
      if (componentItem && !e.target.closest(".remove-component")) {
        const id = componentItem.dataset.id;
        // Optional: visual feedback
        componentItem.style.transform = "scale(0.95)";
        setTimeout(() => (componentItem.style.transform = ""), 150);

        addComponentToCanvas(id);
  ...
  function renderComponents() {
    ...
    components.forEach(function (component) {
      html +=
        '<div class="component-item" draggable="true" data-id="' +
        component.id +
        '">' +
  ...
  function renderCanvas() {
    ...
    droppedComponents.forEach(function (comp, index) {
      html +=
        ...
        '<button class="btn btn-ghost btn-icon remove-component" data-index="' +
        index +
        '" aria-label="Remove ' +
        comp.name +
        '">' +
  ...
  function saveLayout() {
    localStorage.setItem(
      "digital-mira-store-layout",
      JSON.stringify(droppedComponents)
    );
    showNotification("Layout saved successfully!", "success");

    if (
      typeof AuthSimulator !== "undefined" &&
      AuthSimulator.isAuthenticated()
    ) {
      AuthSimulator.updateProgress(
        "udyam",
        Math.min(100, droppedComponents.length * 12.5)
      );
    }
  }
  ...
  function showNotification(message, type) {
    ...
    notification.style.cssText =
      "position: fixed; top: 1rem; right: 1rem; z-index: 9999; animation: modalSlideIn 0.3s ease;";
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    document.querySelector(".store-builder-content") ||
